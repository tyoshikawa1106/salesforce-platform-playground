...
このページは、GitHub Actions、Dependabot、リリースノート、Issue／PRテンプレートの設定内容を説明します。

## .github/workflows/ci.yml

PRと`main`へのプッシュで実行するCIです。

| 設定 / step                                       | 内容                                                                                           |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `pull_request` / `push`                           | `main`向けPRと`main`へのプッシュで実行する。                                                   |
...
リリースノートはdeployやpackage publishを実行する設定ではありません。
...
| ファイル             | 内容                          |
| -------------------- | ----------------------------- |
| `config.yml`         | Issueテンプレート全体の挙動。 |
| `01_improvement.yml` | 機能追加 / 改善 Issue。       |
| `02_bug_report.yml`  | 不具合報告 Issue。            |

必須項目、初期ラベル、秘密情報への注意書きがGitHub運用ルールと一致しているか確認します。
