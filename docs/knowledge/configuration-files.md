...
| `manifest/rebuild-developer-org.xml`          | Dev 組織への標準 validate / deploy scope を定義する。                                                                                    | 追加前に対象 org で validate し、`docs/knowledge/force-app-deployability-inventory.md` と合わせる。            |
...
| `manifest/destructiveChanges.xml`             | Dev 組織や Scratch Org から削除する metadata を Salesforce 標準の destructive changes 形式で定義する。                                   | 実行前に対象 org と削除 scope を確認し、`docs/deployment/salesforce-org-destructive-changes-rules.md` に従う。 |
