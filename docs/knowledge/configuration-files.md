...
| `scripts/deploy/scratch-org/scratch-org.json` | Scratch Org 準備・削除スクリプトで使う alias prefix、duration、manifest、Permission Set、import plan、wait を定義する。                  | 個人専用 alias や秘密情報を入れず、Scratch Org 手順と一致しているか確認する。                                       |
