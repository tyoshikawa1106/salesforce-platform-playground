...
## pre-commit hook との違い

pre-commit hook は、コミットするときにローカルで動きます。

このリポジトリでは、staged files に対して `prettier --write` が実行されます。つまり、コミットしようとしたファイルは、その場で自動整形されます。

ただし、pre-commit hook だけでは次のようなケースを完全には拾えません。

- hook が入っていない環境でコミットされた。
- `--no-verify` で hook をスキップした。
...
PR や `main` push のたびに、GitHub Actions が整形確認を実行します。
...
また、CI は自動修正しません。自動修正まで CI に任せると、PR ブランチへの push 権限や bot commit の扱いが必要になり、運用が複雑になります。まずは check のみで十分です。
