...
Trailhead Playground などの学習用組織は、初期パッケージ、サンプル設定、標準プロファイル、組織固有の接続設定が混ざりやすいため、広い `package.xml` の retrieve 結果をそのまま正本にしないようにします。

このメモでは、Dev 組織から別の Dev 組織へ再現する用途を前提にします。管理対象にすると決めた metadata ディレクトリは、個別ファイルを部分選別せず、原則として取得結果をまとめて管理します。

## 基本方針

| 方針         | 内容                                                                                              |
| ------------ | ------------------------------------------------------------------------------------------------- |
| 管理する     | Apex、LWC、オブジェクト、画面、Settings など、Dev 環境として別組織へ再現したいメタデータ          |
