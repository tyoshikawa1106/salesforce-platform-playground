...
同じIssueに対して、Project、Milestone、ラベルは同時に使えます。

例:

- Project: `Platform Playground`
- Milestone: `v1.0`
- ラベル: `documentation`, `area:github`

この場合、Projectは「今どの状態か」、Milestoneは「どの区切りで終えるか」、ラベルは「何の作業か」を表します。
...
期限やリリースがない場合は、Milestoneを付けずにProjectとラベルで管理する方が自然です。
