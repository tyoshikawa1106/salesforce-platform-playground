...
本文中の「現時点」や「現在」は、特記がなければその判断を書いた時点を指します。
後続の導入状況は、当時の判断を消さずに「その後の更新」として追記します。
...
2026-06-20 に PR #72 で `code-analyzer.yml` を追加済みです。
...
一方で、この判断時点の CI は `npm ci`、`npm audit --omit=dev`、lint、LWC unit test を実行していましたが、format check は実行していませんでした。
...
2026-06-20 に PR #73 で `.github/workflows/ci.yml` の `npm checks` に `npm run prettier:verify` を追加済みです。
...
2026-06-20 に PR #74 で `@sa11y/jest`、`jest-sa11y-setup.js`、`jest.config.js` の setup file 登録を追加済みです。
2026-06-20 に PR #76 で、`sa11y` が Salesforce 社公開の OSS であり、Salesforce 組織接続やメタデータ操作のツールではないことを `docs/development/lwc-jest-accessibility-testing.md` に追記しました。
...
一方で、この判断時点の CI は `npm ci`、`npm audit --omit=dev`、lint、LWC unit test を実行するだけでした。Codecov への upload はなく、Apex coverage も GitHub Actions 上では扱っていませんでした。
...
DreamHouse の依存バージョンへ手動で揃える変更は、現時点では行いません。
...
CodeTour を追加する理由は、リポジトリ内に walkthrough や onboarding tour を用意する場合です。現時点ではその運用がないため、追加しません。
...
DreamHouse に合わせる release 設定は、現時点ではありません。
...
- Code Analyzer 設定: 2026-06-20、PR #72。
- CI の Prettier check: 2026-06-20、PR #73。
- LWC Jest + accessibility: 2026-06-20、PR #74。
- `sa11y` の提供元と用途の補足: 2026-06-20、PR #76。
