...
この文書で画面 LWC と呼ぶ対象は、Lightning ページなどへ公開する LWC、またはデータ取得、利用者操作から結果表示までの処理順序、複数の画面状態、子コンポーネント間の連携を制御する Controller 相当の LWC です。

親から受け取った値の表示、または利用者操作の親への通知だけを担当し、画面単位の処理を制御しない内部子 LWC は表示部品として区別します。これは処理量による例外ではなく、Controller の責務を持つかどうかによる分類です。
...
画面 LWC の対象範囲が曖昧なまま 2 層構成を必須にすると、値の表示や操作の通知だけを行う内部子 LWC にも、意味のある処理を持たない Logic を作る可能性があります。一方で、「単純だから」という理由で画面 LWC を対象外にすると、構成判断のばらつきが再発します。

Lightning ページなどへ公開する LWC と、データ取得、画面処理の順序、複数の状態、子コンポーネント間の連携を制御する LWC を画面 LWC とします。親から受け取った値の表示または操作の通知だけを行う内部子 LWC は表示部品とし、画面 LWC には含めません。対象となる画面 LWC は、処理量にかかわらず 2 層構成とします。
...
- 親から受け取った値の表示、または利用者操作の親への通知だけを担当する内部子 LWC。
- 複数コンポーネントへ関数を公開する API module。
- CSS だけを公開する CSS module。
- Jest test 用の mock。
- インストール済み・生成済みコンポーネント。

表示部品は、処理量が少ないからではなく、画面単位の処理を制御する Controller の責務を持たないため対象外とします。対象外を「単純な画面 LWC」へ広げません。処理量による例外を設けると、構成判断のばらつきが再発するためです。
