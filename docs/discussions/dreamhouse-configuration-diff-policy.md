...
このメモは、当時の判断過程を残すための discussion です。後続の導入状況は、当時の判断を消さずに「その後の更新」として追記します。
...
PR #76 で、`sa11y` が Salesforce 社公開の OSS であり、Salesforce 組織接続やメタデータ操作のツールではないことを `docs/development/lwc-jest-accessibility-testing.md` に追記しました。
...
このリポジトリでは、2GP package を作成・配布する方針はありません。`force-app` を正本として、Dev 組織へ deploy / validate できる状態を重視します。
...
`CODEOWNERS` は review owner を固定する仕組みです。このリポジトリは個人運用で、`docs/development/github-rules.md` でも CODEOWNERS 必須レビューは保留しています。
