...
当初は `.agents/` と `skills-lock.json` を環境ごとに導入するローカル生成物とみなし、Git 管理から除外していました。この方法では、リポジトリの容量や更新差分を抑えられる一方、環境ごとに Skills の有無や内容が異なり、リポジトリを取得した環境で同じ Skills の状態を再現できません。
