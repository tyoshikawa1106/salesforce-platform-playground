...
- マージ後にProjectステータス更新と作業ブランチ整理を行う。Salesforceへの実deployは、ユーザーが別途組織反映を明示した場合だけ行う。
...
Salesforceメタデータ変更を含むPRは、push前に [組織操作ルール](../deployment/org-operation-rules.md#push-前の検証) に従って、対象組織に応じたvalidateまたはdry-runを実行します。対象org、組織種別、deploy可能な変更、scope、実行結果を確認できない場合はpushしません。PR上でdeploy対象metadataを修正した場合も、再push前に必要な検証を行います。

### Salesforce実deploy

PR前は実deployを標準手順にせず、対象組織に応じた限定scopeのvalidateまたはdry-runまでを行います。PRの作成・マージ依頼は実deployの依頼を意味しません。実deployは、ユーザーが別途組織反映を明示した場合だけ、[組織操作ルール](../deployment/org-operation-rules.md#明示依頼を受けた実-deploy) に従って行います。
