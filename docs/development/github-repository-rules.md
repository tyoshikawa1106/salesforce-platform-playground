...
Salesforceメタデータ変更を含むPRは、マージ前に [組織操作ルール](../deployment/org-operation-rules.md#pr-マージ前とマージ後) に従って、対象組織に応じたvalidateまたはdry-runを実行します。対象org、組織種別、deploy可能な変更、scope、実行結果を確認できない場合はマージしません。

### PR マージ後のSalesforce deploy

Salesforceメタデータを開発中に動作確認する場合は、作業ブランチから限定scopeで実deployします。PRのマージ依頼では、[組織操作ルール](../deployment/org-operation-rules.md#pr-マージ前とマージ後) に従い、マージ前に確認した同一scopeだけを、同期済みでクリーンな`main`から最終deployします。
