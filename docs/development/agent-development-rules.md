...
- ファイル変更、コミット、push、PR 作成、merge、deploy、retrieve、delete、Apex test、data import など、作業ツリーや外部状態を変える前に、何をするかを短く提示してユーザー確認を待つ。
- 提示内容には、変更範囲、主な作業、変更予定ファイルや metadata、実行予定の確認コマンド、未確定の前提を含める。
- `git status`、現在ブランチ確認、関連ファイルや docs の read-only 調査、Issue / PR 状態確認は、確認ステップの内容を具体化するために先に実行してよい。
- ユーザーが確認または進行を明示するまで、変更作業や外部状態変更を開始しない。
- 確認後に作業範囲、対象 org、実行コマンド、リスクが変わる場合は、変更を続ける前に再確認する。
