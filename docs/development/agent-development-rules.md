...
### Node.js script の実装コメント

- `scripts/` 配下で独自実装する本体 JavaScript は、新規行や変更行だけでなくファイル全体を対象にコメント規約へ合わせる。
- 宣言、代入、分岐、返却、呼び出し、データ変換など、意味を持つコード行ごとに、目的、成立させる状態、判断理由のいずれかを示す短い日本語コメントを直前へ記載する。
- import、export、空行、括弧や閉じ記号だけの行、複数行式の継続行、設定値を列挙するだけの行は、1 行ごとの日本語コメントの対象外とする。
- Node.js test は 1 行ごとのコメント対象外とし、複雑な準備、実行、検証の意図がコードだけでは判別しにくい場合に日本語コメントを記載する。
- コメント追加だけで読みやすくならない重複処理や複数責務がある場合は、共通化または責務分割の必要性を先に検討する。単純な処理へ説明用 wrapper を追加しない。
