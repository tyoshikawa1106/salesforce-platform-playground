...
- `code-analyzer-results/*`: Salesforce Code Analyzer のローカル解析出力。JSON / log / HTML など、解析結果として再生成できるファイルは Git 管理しない。`code-analyzer-results/code-analyzer-results-guide.md` だけは出力先フォルダ名を固定するため Git 管理する。
