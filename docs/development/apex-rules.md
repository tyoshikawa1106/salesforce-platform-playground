...
- UI、Flow、API などユーザー操作から呼ばれる処理では、SOQL に `WITH USER_MODE`、DML に `AccessLevel.USER_MODE` を使えるか確認する。
...
対象 object、対象 event、handler method は作業対象に合わせて変えます。Trigger に書くコメントは「どの業務処理へ委譲しているか」が分かる短い説明に留め、処理の詳細説明は handler / service 側へ置きます。
...
```sh
sf apex run test --class-names MyServiceTest,MyOtherServiceTest --result-format human --wait 30
```
