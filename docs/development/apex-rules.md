...
ApexDoc は `/** ... */` 形式で書き、対象のクラス、インターフェース、列挙型、メソッド、コンストラクタ、プロパティ、メンバー変数、トリガーの直前に置きます。Apex コンパイラは ApexDoc のタグや説明内容を検証しないため、実装を変更したら対応する ApexDoc も必ず見直します。

### 必須範囲

- `public` / `global` のクラス、インターフェース、列挙型には ApexDoc を付ける。
- `public` / `global` のメソッドとコンストラクタには ApexDoc を付ける。
- LWC、Aura、Flow、REST、Agentforce、パッケージ利用者など外部境界から呼ばれる Apex には、呼び出し側が守るべき契約を明記する。
- `public` / `global` のプロパティと、公開 API の一部になるメンバー変数には ApexDoc を付ける。
- `private` / `protected` のヘルパーでも、複雑な前提、例外、権限境界、拡張ポイントを持つ場合は ApexDoc を付ける。
- テストクラスやテストメソッドは、公開 API ドキュメントではなくテスト仕様として必要な範囲で説明する。全メソッドへ機械的に長い ApexDoc を付けない。

### 基本構成

ApexDoc の先頭には、対象要素を一文で要約する主要説明を書きます。最初の文は生成ドキュメントの一覧や索引に使われる前提で、短く具体的に書き、ピリオドで終えます。

必要に応じて、主要説明の後に前提条件、事後条件、権限境界、null の扱い、一括処理前提、副作用、関連するメタデータや設定を補足します。実装手順を逐語的に説明せず、呼び出し側が知るべき契約を書くことを優先します。

```apex
/**
 * 指定された顧客リクエストから取引先を作成します。
 *
 * DML の前に必須項目を検証し、作成に成功したレコードだけを返します。
 *
 * @param requests 取引先作成リクエスト。各リクエストには空でない名前が必要です。
 * @return 作成された取引先レコード。requests が空の場合は空のリストを返します。
...
`@param` には、型を繰り返すだけでなく、目的、必須/任意、null 可否、空コレクションの扱い、ID 形式、許可値、件数前提を書きます。

`@return` には、戻り値の意味、null を返すか、空コレクションを返すか、順序が保証されるか、部分成功時に何が入るかを書きます。
...
メソッドとコンストラクタでは、呼び出し条件、引数制約、戻り値、例外、副作用、権限境界を書きます。SOQL / DML / callout の有無、一括処理前提、部分成功の扱いが呼び出し側に影響する場合は明記します。

プロパティとメンバー変数では、用途、デフォルト値、初期化後に変更可能か、公開 API として利用者が依存してよい値かを書きます。

トリガーには、ApexDoc 固有のトリガー仕様はありません。トリガー本体はコンテキスト分岐とハンドラー呼び出しに集中させ、実質的な契約はハンドラー / サービスクラス側に書きます。トリガーに ApexDoc を付ける場合は、`@since` や `@see` など短い補足に留めます。
