...
- Map やコレクションの生成、値の設定、検索結果の集計や変換など、SOQL の発行以外のデータ加工は `Service` で行う。
- `Service` から `Selector` を直接呼び出さない。SOQL が必要なデータは `Controller` が `Selector` から取得し、`Service` へ引数で渡す。
- `Selector` で行う処理は SOQL の発行と、取得した検索結果を加工せずに返すことに限定する。
...
- 検索条件値が存在しない場合は、`Selector` で SOQL を実行せず、0件取得と同じ空の List を返す。このクエリ実行可否の確認は、SOQL を発行しないためのガードとして `Selector` で行ってよい。
- 検索条件値の未指定は、`null`、空文字、空白文字、空の List、空の Set とする。数値の `0` と Boolean の `false` は有効な検索値として扱い、SOQL を短絡しない。
- 検索条件値が未指定の場合に空の検索結果を返すための List 生成は、データ加工ではなくクエリ短絡の一部として `Selector` で行ってよい。
- `Selector` では describe、DML、入力値の正規化や判定、Map への値設定、検索結果の集計や変換、返却用データの組み立てを行わない。
...
SObject とコレクションの戻り値や内部処理の初期値では、データがない状態を `null` で表しません。
...
- 非 null を返すことが保証されたメソッドの戻り値は、事前に空のコレクションを生成せず、コレクション型の変数へ直接代入してよい。
- コレクション型の戻り値は `null` を返さず、対応する空のコレクションを返す。
- Salesforce の項目値や外部入力など、呼び出し元から渡される値は `null` になり得るため、必要な境界で明示的に確認する。
