...
ApexDoc は `/** ... */` 形式で書き、対象の class、interface、enum、method、constructor、property、member variable、trigger の直前に置きます。Apex コンパイラは ApexDoc のタグや説明内容を検証しないため、実装を変更したら対応する ApexDoc も必ず見直します。

### 必須範囲

- `public` / `global` class、interface、enum には ApexDoc を付ける。
- `public` / `global` method と constructor には ApexDoc を付ける。
- LWC、Aura、Flow、REST、Agentforce、package subscriber など外部境界から呼ばれる Apex には、呼び出し側が守るべき契約を明記する。
- `public` / `global` property と、公開 API の一部になる member variable には ApexDoc を付ける。
- `private` / `protected` helper でも、複雑な前提、例外、権限境界、拡張ポイントを持つ場合は ApexDoc を付ける。
- test class や test method は、公開 API docs ではなくテスト仕様として必要な範囲で説明する。全メソッドへ機械的に長い ApexDoc を付けない。

### 基本構成

ApexDoc の先頭には、対象要素を一文で要約する main description を書きます。最初の文は生成ドキュメントの一覧や索引に使われる前提で、短く具体的に書き、ピリオドで終えます。

必要に応じて、main description の後に前提条件、事後条件、権限境界、null の扱い、bulk 前提、副作用、関連する metadata や設定を補足します。実装手順を逐語的に説明せず、呼び出し側が知るべき契約を書くことを優先します。

```apex
/**
 * Creates accounts for the requested customer records.
 *
 * The method validates required fields before DML and returns only successfully inserted records.
 *
 * @param requests Account creation requests. Each request must include a non-blank name.
 * @return Inserted Account records. Returns an empty list when requests is empty.
 * @throws AccountServiceException if validation fails or DML cannot be completed.
 */
...
| タグ             | 用途                                                                      |
| ---------------- | ------------------------------------------------------------------------- |
| `@param`         | method / constructor の引数を説明する。実引数と同じ名前、同じ順序にする。 |
| `@return`        | 戻り値を説明する。`void` method と constructor には書かない。             |
| `@throws`        | 明示的に throw する重要な例外と、その発生条件を説明する。                 |
| `@example`       | 呼び出し例や拡張例があると誤用を防げる場合に書く。                        |
| `@deprecated`    | 非推奨理由、非推奨になった version、代替手段を書く。                      |
| `@see`           | 関連 class、method、外部仕様、metadata への参照を書く。                   |
| `@since`         | 導入 version や日付を書く。package や公開 API では優先して使う。          |
| `@version`       | class、interface、enum の version を書く。                                |
| `@group`         | 生成ドキュメント上の分類を書く。                                          |
| `{@link ...}`    | 本文中で class、method、property などへリンクする。                       |
| `{@code ...}`    | 本文中や `@example` 内で code として表示する。                            |
| `{@literal ...}` | HTML や記号を解釈させず文字として表示する。                               |
| `{@hidden ...}`  | 生成ドキュメントに出さない保守者向け情報を書く。                          |

`@param` には、型を繰り返すだけでなく、目的、必須/任意、null 可否、空 collection の扱い、ID 形式、許可値、件数前提を書きます。

`@return` には、戻り値の意味、null を返すか、空 collection を返すか、順序が保証されるか、部分成功時に何が入るかを書きます。

`@throws` には、例外型だけでなく、どの条件で投げるかを書きます。内部実装で捕捉して利用者に見せない例外をすべて列挙する必要はありません。

### 要素別ルール

class では、責務、共有モデル、主な呼び出し元、重要な副作用を書きます。`without sharing` や `inherited sharing` を使う場合は、なぜその共有モデルが必要かを ApexDoc または近接する設計説明で分かるようにします。

interface では、実装詳細ではなく contract を書きます。実装クラスが満たすべき期待動作、入力、戻り値、例外、bulk 対応の有無を明確にします。

enum では、enum 全体が表す概念を書きます。各値の意味が名前だけで明らかでない場合は、値の直前に短いコメントを置きます。

method と constructor では、呼び出し条件、引数制約、戻り値、例外、副作用、権限境界を書きます。SOQL / DML / callout の有無、bulk 処理前提、部分成功の扱いが呼び出し側に影響する場合は明記します。

property と member variable では、用途、default value、初期化後に変更可能か、公開 API として利用者が依存してよい値かを書きます。

trigger には、ApexDoc 固有の trigger 仕様はありません。Trigger 本体は context 分岐と handler 呼び出しに集中させ、実質的な契約は handler / service class 側に書きます。Trigger に ApexDoc を付ける場合は、`@since` や `@see` など短い補足に留めます。

annotation が付く要素では、その annotation が利用者に与える意味を書きます。

- `@AuraEnabled(cacheable=true)` では、cache 前提、更新後の再取得要否、読み取り専用であることを書く。
...
- `@return` を `void` method に書く。
- 実引数に存在しない `@param`、順序が違う `@param` を残す。
- org 固有のユーザー名、メールアドレス、record ID、endpoint、秘密情報を例や説明に書く。
