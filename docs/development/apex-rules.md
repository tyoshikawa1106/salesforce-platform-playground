...
- PMD `AvoidLogicInTrigger` はこの Trigger context 分岐と衝突するため、Trigger 本体の処理が handler 呼び出しだけに限られる場合は、コードコメントを増やさず、対象 trigger を限定した `code-analyzer.yml` の suppression で扱う。
...
- PMD `ApexDoc` が単純な LWC / Aura 用 Wrapper プロパティの `//` コメント方針と衝突する場合は、コード内に不要な ApexDoc を戻さず、対象ファイルを限定した `code-analyzer.yml` の suppression で扱う。
- PMD `AvoidLogicInTrigger` が Trigger 本体の context 分岐と衝突する場合は、対象 trigger を限定した `code-analyzer.yml` の suppression で扱う。
