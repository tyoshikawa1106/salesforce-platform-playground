...
| `applications/`                     | 25         | 対象外            | 標準 app / namespace / タブ構成の前提が強く、標準 Dev scope には入れない。           |
...
| `applications/` / `appMenus/` | 標準 app、tab、namespace、表示順に依存する。                   | カスタム app を作る場合だけ個別に管理する。                                       |
...
| 中     | `applications/` / `appMenus/`            | カスタム app と標準 app を分けると deploy scope を狭められる。 |
