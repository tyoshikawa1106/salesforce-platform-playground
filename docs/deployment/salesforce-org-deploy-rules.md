...
- 現在接続されている Salesforce 組織には source tracking がないため、`sf project deploy preview` は標準の確認手段にしない。
...
- Salesforce 組織への通常検証は `manifest/rebuild-developer-org.xml` を使う。作業範囲がさらに狭い場合は、作業対象 manifest または `--metadata` で絞る。
