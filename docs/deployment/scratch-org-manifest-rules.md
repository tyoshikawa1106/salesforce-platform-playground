...
初期反映のdeployは`RunLocalTests`を必須とし、metadata反映とApexテストが同じdeployで成功した場合だけ初期反映成功として扱います。
...
広い dry-run は通常作業の起点ではなく、調査用です。
Salesforce 組織から大きく retrieve した直後や、Scratch Org 初期反映用 manifest の範囲を見直す場合に使います。
