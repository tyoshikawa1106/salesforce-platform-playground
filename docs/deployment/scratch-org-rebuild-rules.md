...
```sh
node scripts/deploy/scratch-org/scratch-org-create.js
node scripts/deploy/scratch-org/scratch-org-deploy.js
node scripts/deploy/scratch-org/scratch-org-assign-permset.js
node scripts/deploy/scratch-org/scratch-org-import-test-data.js
```
...
alias は `scripts/deploy/scratch-org/scratch-org.json` で指定します。
...
追加情報が必要な場合だけ `sf org display --target-org scratch-platform-playground` を使い、報告には実ユーザー名や org 固有 URL を書きません。
...
```sh
node scripts/deploy/scratch-org/run-destructive-scratch-org.js
```
