...
Installed Package の確認方法と反映順序は、[Scratch Org 再現の前提と設定](scratch-org-rebuild-reference.md#installed-package-の確認)を参照します。
...
## alias と Scratch definition

alias の付け方と Scratch definition の feature 判断は、[Scratch Org 再現の前提と設定](scratch-org-rebuild-reference.md)を参照します。
...
その場合は、作成時に表示された alias を `SCRATCH_ORG_ALIAS` で明示します。

```sh
SCRATCH_ORG_ALIAS=<scratch-org-alias> node scripts/deploy/scratch-org/scratch-org-create.js
SCRATCH_ORG_ALIAS=<scratch-org-alias> node scripts/deploy/scratch-org/scratch-org-deploy.js
SCRATCH_ORG_ALIAS=<scratch-org-alias> node scripts/deploy/scratch-org/scratch-org-assign-permset.js
...
alias、状態、有効期限は `sf org list` で確認します。
追加情報が必要な場合だけ `sf org display --target-org <scratch-org-alias>` を使い、報告には実ユーザー名や org 固有 URL を書きません。
...
```sh
sf project deploy start --dry-run --source-dir force-app --target-org <scratch-org-alias> --wait 30
```
...
Settings や DuplicateRule を Scratch Org 専用に別管理して個別補正すると二重管理になりやすいため、必要な場合は対象 metadata ごとに manifest、scratch definition、または手順 docs で扱います。

### External Client App の扱い
...
```sh
sf project retrieve start --manifest manifest/scratch-work.xml --target-org <scratch-org-alias>
```
...
通常は `node scripts/deploy/scratch-org/run-constructive-scratch-org.js` の実行結果、またはこのページの個別 `sf` コマンドの実行結果で準備内容をまとめます。

組織をブラウザで確認する場合:

```sh
sf org open --target-org <scratch-org-alias> -b chrome
```
...
```sh
SCRATCH_ORG_ALIAS=<scratch-org-alias> node scripts/deploy/scratch-org/run-destructive-scratch-org.js
```

自動化や明示的に削除対象を確認済みの場合は、確認プロンプトを省略できます。

```sh
sf org delete scratch --target-org <scratch-org-alias> --no-prompt
```
