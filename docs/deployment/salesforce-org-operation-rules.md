...
- PR merge 前 preflight と merge 後 deploy
- 実行結果と未実行項目の報告
...
- login URL だけで組織種別や利用可能な preflight を判断しない。対象 org または CI 接続先を変更する場合は、組織種別と利用するコマンドを確認してから運用へ組み込む。
...
- 通常開発では、PR の deploy 可能な変更が選んだ scope にすべて含まれることを確認し、validate と merge 後 deploy に同じ scope を使う。標準 manifest に含まれない変更は、作業対象 manifest または `--metadata` で補う。
...
PR ブランチでは実 deploy せず、merge 前の validate または dry-run を最終 gate として扱います。通常の Apex / Salesforce メタデータ開発では、merge 後に同期した clean な `main` から、preflight を実行した同じ対象 org へ同じ scope で実 deploy します。
...
通常開発で Salesforce メタデータ変更を含む PR を merge した場合は、ローカルの `main` を `origin/main` と同期し、作業ツリーが clean で `HEAD` と `origin/main` が一致することを確認してから、preflight に成功した同じ対象 org へ同じ scope で実 deploy します。Salesforce メタデータ変更を含む PR の merge 依頼は、この merge 後 deploy まで含むものとして扱います。
...
merge 後は deploy report で成功、対象件数、エラー、テスト結果を確認します。org と Git の一致確認が必要な変更では、同じ scope を retrieve し、実質差分がないことも確認します。deploy と必要な一致確認が成功するまで、その Salesforce 開発タスクは完了扱いにしません。deploy に失敗した場合は、原因を修正する follow-up PR または revert の要否を判断し、未反映の状態を報告します。

次の場合は merge 後 deploy の対象外です。
