...
通常開発で Salesforce メタデータ変更を含む PR を merge した場合は、ローカルの `main` を `origin/main` と同期し、作業ツリーが clean で `HEAD` と `origin/main` が一致することを確認してから、validate に成功した同じ対象 org へ同じ scope で実 deploy します。Salesforce メタデータ変更を含む PR の merge 依頼は、この merge 後 deploy まで含むものとして扱います。
