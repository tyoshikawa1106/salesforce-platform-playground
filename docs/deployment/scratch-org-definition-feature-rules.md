...
Dev 組織から retrieve した `force-app/main/default/settings` を、Scratch Org 用にそのまま deploy する方針は避けます。
...
この整理は、Scratch Org を Dev 組織と完全一致させるためではなく、主要な開発・検証対象を再現できる最小限の作成条件を明確にするためのものです。
