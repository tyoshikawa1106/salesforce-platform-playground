...
4. 表示中Caseをアンカー要素のないリンクなし表示で必ず含め、別ケースと合わせた最大20件を`CreatedDate`の降順で表示します。
5. ケース番号、件名、状況、原因、オープン日、取引先、取引先責任者をコンパクトなカードとして表示します。
...
- `CaseNumber`と`CreatedDate`を取得必須項目とし、`Subject`、`Status`、`Reason`、`Account.Name`、`Contact.Name`は任意項目として取得します。
- Apex実行権限は必要ありません。
...
- ケース番号、件名、状況、原因、オープン日、取引先、取引先責任者、別タブで開くレコードリンク
