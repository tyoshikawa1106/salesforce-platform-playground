...
3. 関連ケースを`LastModifiedDate`の降順で5件取得します。
4. 表示中Caseをアンカー要素のないリンクなし表示で先頭に置き、その後へ直近の別ケースを並べて最大5件を表示します。
5. ケース番号、件名、ステータス、最終更新日時をコンパクトなカードとして表示します。
6. 「顧客」タブと「会社」タブを`lightning-tabset`の標準タブで切り替え、カード外周との間に余白を設けます。
7. 表示中Case以外は、ケース番号から対象Caseレコードへ移動できるURLを`NavigationMixin.GenerateUrl`で生成します。

## 出力・更新対象

- 「顧客」タブ: 表示中Caseを先頭に、同じContactに紐づく別ケースを含めて最大5件表示します。
- 「会社」タブ: 表示中Caseを先頭に、同じAccountに紐づく別ケースを含めて最大5件表示します。
- レコードの作成、更新、削除は行いません。
...
- 表示中Caseを先頭へ含めてリンクなしで表示し、別Caseと合わせて最大5件に制限すること
