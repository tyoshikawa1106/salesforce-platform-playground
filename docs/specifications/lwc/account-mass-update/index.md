...
法人取引先を検索し、一覧の各行で所有者・業種・種別・評価を編集してまとめて保存するLWCです。専用タブ「取引先一括更新」から利用します。
...
未確認：Chromeでの所有者検索・保存操作は、開発組織へ反映後に確認します。個人取引先が有効な組織での除外分岐は、対応する組織での検証が必要です。タブの追加先アプリは未確定です。
