...
- ファイル全件の照会には`PermissionsQueryAllFiles`も必要。非共有ファイルを見落とした状態で全件削除と扱わない。
...
受付したジョブはIDで追跡し、最新ジョブへ置き換えません。受付ジョブは連続実行の最初の一件なので、その終了を全体完了と表示しません。後続の停止理由は設定のApexジョブで確認します。
...
- `TestDataDeleteServiceTest`: 対象外照会、全ファイル参照権限、実行環境。
