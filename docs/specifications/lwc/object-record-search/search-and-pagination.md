...
- 許可されていないソート項目、壊れたページトークン、ソート項目または方向が現在の条件と一致しないトークンは検索エラーにします。
...
- ページトークンはソート項目、方向、最終行のソート値とIdをJSONとしてシリアライズした内部カーソルです。署名や暗号化は行わず、Apexで構造と現在のソート条件との一致を検証します。
