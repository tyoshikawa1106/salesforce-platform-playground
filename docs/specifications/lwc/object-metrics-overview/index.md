...
- `ObjectMetricsOverviewController` と関連 Apex クラスの実行権限が必要です。
...
- Apex テストでカタログ順、実件数、0件、参照不可相当、不明な API 名、50,000件上限を確認します。
- Jest テストで初期表示、読込中、件数表示、上限表示、エラー、再読み込み、カード選択、検索画面からの復帰を確認します。
