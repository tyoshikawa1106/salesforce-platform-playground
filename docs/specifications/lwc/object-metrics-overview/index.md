...
| LWC JavaScript | `objectMetricsOverviewLogic.js` | 件数応答とカード表示モデルの生成                   |
...
手動更新と子の `recordschanged` 通知は同じ再取得処理を使用します。再取得失敗はデータボードに表示し、子の保存・削除の成功とは区別します。次の再取得成功時にエラーを解除します。
