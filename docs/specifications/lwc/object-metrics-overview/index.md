...
| LWC          | `errorUtils`                                                              | Apexエラーの画面表示用メッセージ変換                 |
...
- `ObjectMetricsOverviewController` と関連Apexクラスの実行権限が必要です。
...
- Lightningホームページ
- `ObjectMetricCatalog` を共有する検索、削除処理

## テスト・確認観点

- Apexテストでカタログ順、実件数、0件、参照不可相当、不明なAPI名、50,000件上限を確認します。
- Jestテストで初期表示、読込中、件数表示、上限表示、エラー、再読み込み、カード選択、検索画面からの復帰を確認します。
