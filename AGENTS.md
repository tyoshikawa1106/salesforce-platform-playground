...
- キーチェーンアクセスなどの OS の資格情報管理画面を起動・操作せず、`security` コマンドの実行、CLI の認証ファイルや保存済み資格情報の直接参照・調査を行わない。
- 通常の開発作業に必要な `sf`、`gh` コマンドは使用してよい。ただし、保存済み資格情報の実値を表示・抽出するコマンドやオプションは使用しない。実行中にパスワード要求、キーチェーン関連の画面またはエラーが出た場合は、その時点で停止し、権限要求、回避操作、資格情報の抽出を行わず、実行コマンドと発生事象だけを報告する。
